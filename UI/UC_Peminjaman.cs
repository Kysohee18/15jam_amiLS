    ...
    namespace Ucp_pabd_lab.UI
{
    public partial class UC_Peminjaman : Form
    {
        ...
        private void LoadComboBoxBarang()
        {
            try
            {
                DataTable dt = db.GetBarangTersedia();
        ...
        private void btn_proses_pinjam_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                string idBarang = cmb_pinjam_barang.SelectedValue.ToString();
                string idUser = cmb_pinjam_user.SelectedValue.ToString();

                // 1. Eksekusi Peminjaman Pokok (Memotong stok dan menambah tanggungan) melalui DAL
                db.InsertPeminjaman(idBarang, idUser);

                // NOTE: Pencatatan Log PINJAM sebelumnya diproses manual di sini, 
                // sekarang di pindah ke database 
